		...
		for {
			...
			if editor.textArea != nil {
				...
				if editor.textArea.buffer != nil && time.Since(editor.textArea.lastActivity) > autoClearDuration {
					...
					editor.clearEditor()
					fyne.Do(func() {
						dialog.ShowInformation("", "Конфиденциальные данные\nстёрты из-за бездействия", editor.window)
					})
				} else {
					editor.textArea.mu.Unlock()
				}
...
func (e *SecureEditor) showInfoPopup() {
	...
	projectLink := widget.NewHyperlink("Проект с открытым исходным кодом", projURL)
	...
	content := container.NewVBox(
		...
		widget.NewLabelWithStyle("выпущен под лицензией Apache 2.0", fyne.TextAlignCenter, fyne.TextStyle{}),
...
func (e *SecureEditor) setupMobileUI() fyne.CanvasObject {
	...
	e.textArea.SetPlaceHolder("Введите текст...")

	encryptBtn := widget.NewButton("Зашифровать", e.encryptText)
	encryptBtn.Importance = widget.HighImportance
	decryptBtn := widget.NewButton("Расшифровать", e.decryptText)
	decryptBtn.Importance = widget.HighImportance
	clearBtn := widget.NewButton("Очистить", e.clearEditor)
	clearBtn.Importance = widget.MediumImportance

	selectAllBtn := widget.NewButton("Выделить всё", e.selectAll)
	selectAllBtn.Importance = widget.MediumImportance
	copyBtn := widget.NewButton("Копировать", e.copyToClipboard)
	copyBtn.Importance = widget.MediumImportance
	pasteBtn := widget.NewButton("Вставить", e.pasteFromClipboard)
...
func (e *SecureEditor) copyToClipboard() {
	...
	if text == "" {
		dialog.ShowInformation("", "Нечего копировать", e.window)
		return
	}
...
func (e *SecureEditor) pasteFromClipboard() {
	...
	if text == "" {
		dialog.ShowInformation("", "Буфер обмена пуст", e.window)
		return
	}
...
func (e *SecureEditor) askPassword(callback func(*memguard.LockedBuffer, error)) {
	...
	formItems := []*widget.FormItem{widget.NewFormItem("Пароль", passEntry)}

	dlg := dialog.NewForm("", "OK", "Отмена", formItems, func(confirmed bool) {
		if !confirmed {
			callback(nil, errors.New("отменено"))
			return
		}
		if len(passEntry.Text) < 15 {
			dialog.ShowInformation("", "Пароль слишком короткий\nТребуется минимум 15 символов", e.window)
			return
		}
...
func (e *SecureEditor) clearEditor() {
	if e.textArea.GetText() == "" {
		dialog.ShowInformation("", "Текстовое поле уже пустое", e.window)
		return
	}
	e.cleanup()
}
...
func (e *SecureEditor) encryptText() {
	...
	if text == "" {
		dialog.ShowInformation("", "Введите текст для шифрования", e.window)
		return
	}
...
func (e *SecureEditor) decryptText() {
	...
	if text == "" {
		dialog.ShowInformation("", "Вставьте зашифрованный текст\nдля расшифровки", e.window)
		return
	}
	...
	e.askPassword(func(passphrase *memguard.LockedBuffer, err error) {
		...
		if e.decryptAttempts >= maxDecryptAttempts {
			e.mu.Unlock()
			dialog.ShowError(errors.New("превышен лимит:\nслишком много неудачных попыток"), e.window)
			return
		}
...
func remove4096Padding(data []byte) ([]byte, error) {
	if len(data) == 0 {
		return nil, errors.New("невозможно снять padding с пустых данных")
	}
